...
- **Picset 生图联动（MVP）**：支持通过网页自动化触发生图、下载结果，并可选一键发布到小红书
...
### 7. Picset 生图并联动发布（MVP）

```bash
# 仅生图 + 下载（需先在浏览器中完成 Picset 登录）
python scripts/picset_automation.py \
    --prompt "奶油色穿搭海报，模特居中，极简留白" \
    --output-dir "C:\temp\picset_out"

# 读取 prompt 文件，并自动发布到小红书（可加 --preview 仅填充）
python scripts/picset_automation.py \
    --prompt-file prompt.txt \
    --publish-to-xhs \
    --title "春季奶油系穿搭模板" \
    --content "今日份氛围感模板，附关键词可复用。#穿搭 #模板" \
    --headless
```

说明：Picset 暂无公开 API 时，脚本使用 CDP 网页自动化方式触发生成。  
说明：MVP 版优先保证“可跑通”，页面改版后可能需要更新按钮/输入框识别逻辑。  
